package com.sktelecom.showme.Main.my.wallet.contribution.Left;
...
public class WalletLeftCont extends PController {
    ...
    public WalletLeftCont(Activity pAct, WalletBodyFrag motherfrag, ICallbackToAct mICallbackToAct) {
        super(pAct);
        this.pAct = pAct;
        this.motherfrag = motherfrag;
        this.mICallbackToAct = mICallbackToAct;
        DBManager.Companion.withDB(pAct).withProperty().getProperty("time");
    }

    public void asViewModelCreate() {
                ...
                try {
                    ...
                    if (isReturn != null && isReturn.equals("true")) {
                        ...
                        for (int i = 0; i < array.length(); ++i) {
                            ...
                            fruitsStringList.add(new VoContents(row.optString("CREATE_USER_NAME"), "type", "desc", row.optString("CREATE_USER_IMG_URL")));
...
//                    frag.getActivity().finish();
    ...
    public WalletCommonVM asViewModelResume() {
        if (vm == null)
            asViewModelCreate();
        return vm;
    }
