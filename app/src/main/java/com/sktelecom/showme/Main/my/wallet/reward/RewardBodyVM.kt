...
class RewardBodyVM : PViewModel() {
    internal val TAG = RewardBodyVM::class.java.simpleName
    internal lateinit var fruitList: MutableLiveData<List<PBean>>


    override fun asFragCreate(): PFragment {
        frag = RewardBodyFrag.with(this, mICallBack);
        return frag
    }
...
// val contentsId: String, val heart: String, val title: String, val artistName: String, val artistId: String, val artistImgUrl: String, val imgUrl: String, val videoUrl: String
                            fruitsStringList.add(VoContents(row.optString("TITLE"), "type", row.optString("TITLE"), row.optString("CREATE_USER_IMG_URL")))
                            fruitsStringList.add(VoContents(row.optString("TITLE"), "type", row.optString("TITLE"), row.optString("CREATE_USER_IMG_URL")))
                        }
                        msg.what = 0
    ...
    internal val mICallBack = object : RewardBodyFrag.ICallbackEvent {
